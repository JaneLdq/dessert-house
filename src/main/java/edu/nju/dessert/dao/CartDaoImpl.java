...
public class CartDaoImpl implements CartDao {
	...
	public boolean addDessert(int uid, int dessertId, int quantity, int storeId, Date date, double price) {
		...
		int result = query.executeUpdate();
		if( result <= 0){
			return baseDao.save(new Cart(uid, dessertId, quantity, storeId, date,price));
		}
		return true;
	}
	...
	public List<CartItemVO> getCartItem(int uid) {
		...
		String hql = "select new edu.nju.dessert.vo.CartItemVO(c.id, c.user_id, c.dessert_id, d.name,"
				+ " s.id, s.name, c.date, c.quantity, c.price, pi.remain)"
	...
	public Double getCartTotalSum(int uid){
		String hql = "select sum(c.price* c.quantity) from Cart c where c.user_id=" + uid;
		Query query = baseDao.getSession().createQuery(hql);
	...
	public Double getCartItemSum(int id) {
		String hql = "select sum(c.price*c.quantity) from Cart c where c.id=" + id;
		Query query = baseDao.getSession().createQuery(hql);
		List<Double> list = query.list();
		return list.get(0);
	}
	...
	public boolean addDessert(int uid, int dessertId, int quantity, int storeId, Date date,
			double price, List<AdditionItemVO> additions) {
		...
		int result = query.executeUpdate();
		if( result <= 0){
			Cart c = new Cart(uid, dessertId, quantity, storeId, date, price);
